/*
    * DDL (DATA DEFINITION LANGUAGE) : 데이터 정의 언어
    ...
    오라클에서 제공하는 객체(OBJECT) = '구조'를
    새로이 만들고(CREATE), 구조를 변경(ALTER), 구조 자체를 삭제(DROP)하는 등
    즉, 실제 데이터 값이 아닌 구조 자체를 정의하는 언어로
    주로 DB관리자, 설계자가 사용함
    
    오라클에서의 객체(구조) : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE),
                           인덱스(INDEX), 패키지(PACKAGE), 트리거(TREGGER),
                           프로지서(PROCEDUAL), 함수(FUNTION),
                           동의어(SYNCNYM), 사용자(USER)
*/
...
/*
    ...
    * 자료형
    ...
    - 문자 (CHAR / VARCHAR2) (크기지정을 반드시 해야만 됨)
      > CHAR : 최대 2000BYTE 까지 저장 가능 / 고정 길이 ( 아무리 저그은 값이 들어와도 처음 할당한 크기 그대로)
...
SELECT * FROM USER_TAB_COLUMNS; -- 여기서 TAB은 TABLE임
...
-- INSERT INTO MEMBER VALUES('ASDFASDFASDFADFASDFASDFASDFASDFASDFASDF','pass03','김철수',syseate);

/*
    < 제약조건 CONSTRAINTS >
    - 원하는 데이터 값만 유지하기 위해서(보관하기 위해서) 특정 컬럼에 설정하는 제약
    - 데이터 무결성 보장을 목적으로 한다.
    - 들어올 데이터에 문제가 없는지 자동으로 검사할 목적
    
    * 종류 : NOT NULL, UNIQUE(중복X), CHECK, PRIMARY KEY, FOREIGN KEY
...
-- 테이블 생성시 컬럼에 제약조건을 거는 방식이 크게 두 가지 (컬럼레벨 / 테이블레벨) 있다.
...
VALUES( 3, 'user02', 'pass03', '홍길녀', '여', '010-1111-2222', 'hgn@gamil.com');
--> UNIQUE제약조건에 위배되었으므로 INSERT실패
--> 오류 구문으로 제약조건명으로 알려줌 ( 컬럼명으로 알려주지 않음) --> 쉽게 팡악하기 어려움
...
/*
    ...
    * 제약조건 부여하는 표현식
    ...
    > 컬럼레벨 방식
    CREATE TABLE 테이블명 (
        컬럼명 자료형(크기) [CONSTRAINT 제약조건] 제약조건,
        컬럼명 자료형(크기),
        ...
    );
...
CREATE TABLE MEM_UNIQUE2 (
    ...
    MEM_PWD VARCHAR(20) NOT NULL,
    MEM_NAME VARCHAR(20) CONSTRAINT MEM_UNIQUE_MEMNAME_NN NOT NULL,
    GENDER CHAR(3),
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(50),
...
INSERT INTO MEM_UNIQUE2
VALUES( 3, 'user03', 'pass03', '홍길녀', '여', '010-1111-2222', 'hgn@gamil.com');
--> 성별에 유효한 값이 아닌게 있어도 잘 INSERT되어버림
...
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1111-2222', 'aaa@nver.com');
...
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) VALUES('user100', 'pass100', '이순신');
--> 지정안된 컬럼에 기본값이 부여되어있었다면 NULL값이 아닌 기본값이 들어가게됨ㄴ
SELECT * FROM MEMBER;
...
--   테이블에서 각 행의 정보를 식별하기 위해 사용할 컬럼에 부여하는 죄약조건
--   --> 각행들을 구분할 수 있는 식별자의 역할 (EX. 회원번호, 사번, 부서코드, 직급코드, 주문번호, 에약번호,...)
--   --> PRIMARY KEY로 제약조건을 하게 되면 해당 그 컬럼에 NOT NULL + UNIQUE 제약조건을 의미
--   주의할 점 : 한 테이블 당 한 개만 설정 가능!
...
CREATE TABLE MEM_PRIMARYKEY2(
    ...
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(50),
    PRIMARY KEY(MEM_NO, MEM_ID)    --> 컬럼 묶어서 기본키로 설정 --> 복합키
);
...
INSERT INTO MEM_PRIMARYKEY2
VALUES(3, NULL, 'pass03', '윤관순', null, null, null);
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER,-- REFERENCES MEM_GRADE -- 생략가능.PRIMARY키로 지정했기 때문에!(GRADE_CODE) --> 컬럼레벨 방식
...
--> NULL 들어갈 수 있음! NOT NULL제약조건 걸어주면 NULL값 안 들어가게 할 수 있다.
...
-- 부모테이블(MEM_GRADE) 의 데이터 값을 삭제했을 경우 문제가 발생할 수 있음
SELECT * FROM MEM;
SELECT * FROM MEM_GRADE;
...
-- 부모테이블의 데이터가 삭제됐을 때 어떻게 처리할 지를 옵션으로 정해놓을 수 있다.
...
DROP TABLE MEM;
-- 1) ON DELETE SET NULL : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식데이터를 NULL값으로 변경시킴
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE ON DELETE SET NULL --부모테이블의 값만 들어올 수 있어! 무모 데이터 삭제되면 자식은 NULL로 기술
);
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10; --> 문제 없이 잘 삭제됨! (단, 10가져다 쓰고 있는 자식데이터 값들이 다 NULL로 변경되었을 것)

SELECT * FROM MEM_GRADE;
SELECT * FROM MEM;

ROLLBACK; -- 원복시키겠다 MEM_GRADE만 원복하는게 아니라 모두 다 원복 되어 MEM테이블의 (INSERT했던) 자료들도 사라짐

DROP TABLE MEM;
-- 2) ON DELETE CASCADE : 부모데이터 삭제 시 해당 데이터를 쓰고 있는 자식 데이터도 같이 삭제함
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10; --> 문제없이 잘 삭제됨 (단, 해당 데이터가 사용하고 있던 자식데이터도 같이 DELETE 됨!

SELECT * FROM MEM_GRADE;
SELECT * FROM MEM;

-- 실습문제 다 풀면 ERDCLOUD 사이트 회원가입 --> ERD만들기 -->

---------------------------------------------------------------------
-- KH계정에서 하기

-- * SUBQUERY를 이용한 테이블 생성(테이블 복사 같은 개념)
...
SELECT * FROM EMPLOYEE_COPY; -- 주석까지는 COPY 안됨.

CREATE TABLE EMPLOYEE_COPY2
AS SELECT *
   FROM EMPLOYEE
   WHERE 1=0; --> 구조만 복사되고 모든 행에 대해 매번 FALSE기 때문에 데이터 값은 복사 안됨.
   
   
SELECT * FROM EMPLOYEE_COPY2;

CREATE TABLE EMPLOYEE_COPY3
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY*12 연봉 --> SELECT절에 산술연산 또는 함수식이 기술된 경우 별칭 반드시!
   FROM EMPLOYEE; 
...
-- FOREIGN KEY : ALTER TABLE 테이블명 ADD FOREIGN KEY(컬럼명) REFERENCES 참조할테이블명[(컬럼명)] --생략하면 기본키;
...
-- DEPARTMENT테이블에 LPCATION_ID에 외래키 제약조건 추가
