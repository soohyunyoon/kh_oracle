...
/*
    *서브쿼리 구분
     서브쿼리를 수행한 결과 값이 몇행 몇열이냐에 따라서 분류됨
    
    - 단일행 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 오로지 1개일 때
    
    - 다중행 서브쿼리 : 서브쿼리의 조회 결과 값의 행수가 여러 행일 때
    
    - 다중열 서브쿼리 : 서브쿼리의 조회 결과 값이 한 행이지만 컬럼이 여러개일 때
    
    - 다중행 다중열 서브쿼리 : 서브쿼리의 조회 결과 값이 여러행 여러열일 때 
    
    >> 서브쿼리 유형에 따라 서브쿼리 앞에 붙는 연산자가 달라짐
*/

/*
    1. 단일행 서브쿼리 (SINGLE ROW SUBQUERY)
       서브쿼리의 조회 결과값의 개수가 1개일 때 (단일행 단일열)
       일반 연산자 사용
       = != ^= < > ,...
*/
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALARY)      --> 결과 값 1행 1열
                FROM EMPLOYEE);
                
-- 3) 노옹철 사원의 급여보다 많이 받는 사원의 사번, 이름, 부서코드, 직급코드, 급여
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > (SELECT SALARY
                FROM EMPLOYEE
                WHERE EMP_NAME = '노옹철');     --> 결과 값 1행 1열

-- * 서브쿼리는 WHERE 절 뿐만 아니라, SELECT / FROM / HAVING 다양한 곳에서 사용가능
...
/*
    2. 다중행 서브쿼리 (MULTI ROW SUBQUERY)
       ...
       - IN ('값1', '값2', ...) / NOT IN 서브쿼리 : 여러 개의 결과 값 중에서 한 개라도 일치하는 값이 있다면 혹은 없다면 이라는 의미
       
       - > ANY / < ANY 서브쿼리 : 서브쿼리 실행 후 여러개의 결과 값 중에서 한 개라도 큰 / 작은 경우
                                 여러 개의 값들 중 가장 작은 값 보다 크냐? / 가장 큰 값 보다 작냐?
 
       - > ALL / < ALL 서브쿼리 : ALL을 붙였을 경우 여러개의 결과 값의 모든 값보다 큰 / 작은 경우 
                                가장 큰 값보다 크냐? / 가장 작은 값 보다 작냐?
...
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY IN (SELECT MAX(SALARY) --> 결과 값 여러행 1열
                 FROM EMPLOYEE
                 GROUP BY DEPT_CODE);
...
-->> 위와 결과물을 하나의 결과로 보고 싶으면 (UNION 방식)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '사수' "구분"
FROM EMPLOYEE
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID
                 FROM EMPLOYEE
...
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '과장'
  AND SALARY > ALL (SELECT MAX(SALARY) -- ANY나 MAX나 둘 중 하나 써도 값이 나옴
                    FROM EMPLOYEE
                    JOIN JOB USING (JOB_CODE)
                    WHERE JOB_NAME = '차장');
...
-- 2) 박나라 사원과 직급코드가 일치하면서 같은 사수를 가지고 있는 사람
-- 사번, 이름, 직급코드, 사수사번
SELECT EMP_ID, EMP_NAME, JOB_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE (JOB_CODE, MANAGER_ID) = (SELECT JOB_CODE, MANAGER_ID --> 결과 값 1행 여러 열 / 1행이므로 = 사용. 여러행이면 IN사용
                                FROM EMPLOYEE
                                WHERE EMP_NAME = '박나라');
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (SELECT JOB_CODE, MIN(SALARY) -- 여러개의 값을 출력하고 싶으면 IN
                            FROM EMPLOYEE
                            GROUP BY JOB_CODE);
...
FROM EMPLOYEE
WHERE DEPT_CODE IS NULL AND SALARY = 2320000 -- NULL값 비교는 IS NULL로 해야 제대로 비교가 됨.
...
-- 사번, 이름, 연봉, 부서코드
-- 연봉이 3000만원 초과인 사원만 조회
SELECT EMP_ID, EMP_NAME, SALARY * 12 연봉, DEPT_CODE
FROM EMPLOYEE
WHERE SALARY * 12 > 30000000; -- 별칭X FROM - WHERE - SELECT 순
...
-- 2) 부서별 평균급여 높은 3개의 부서의 부서코드, 평균 급여 조회
...
RANK() OVER(정렬기준)       /   DENSE_PANK() OVER(정렬기준)

- RANK() OVER(정렬기준) : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너띄고 순위 계산
                         EX) 공동 1위가 2명이면 그 다음 순위를 3위
