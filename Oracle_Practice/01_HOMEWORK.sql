...
-- 선수과목이 존재하는 과목들은 어떤 과목인지 과목번호를 조회해보시오 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>다시 풀어볼 것
SELECT CLASS_NO
FROM TB_CLASS
...
-- 나이 어린 ~ 많은 / 2000년 이후 출생자X /헤더 교수이름, 나이 / 나이는 만으로 계산 >>>>>>>>>>>>>>>>>>>>>>>>>>>다시 풀어볼 것
...
-- 12. 학번이 A112113인 김고운 학생의 년도 별 평점을 구하는 SQL문 >>>>>>>>>>>>>>>>>>>> 다시 풀어볼 것
SELECT SUBSTR(TERM_NO,1,4) 년도, ROUND(AVG(POINT),1) "년도 별 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY SUBSTR(TERM_NO,1,4);

-- 학과 별 휴학생 수를 파악하고자 한다. 학과 번호와 휴학생 수를 표시하는 SQL문장을 작성 >>>>>>>>>>>>>>>>>>> 다시 풀어볼 것
SELECT DEPARTMENT_NO 학과, COUNT(DECODE(ABSENCE_YN,'Y','N')) "휴학생 수"
FROM TB_STUDENT
GROUP BY DEPARTMENT_NO
ORDER BY DEPARTMENT_NO;
...
-- 학번이 A112113인 김고운 학생의 년도, 학기 별 평점과 년도 별 누적 평점,
-- 총 평점을 구하여라(단, 평점을 구하는 SQL구문을 작성하여라 >>>>>>>>>>>>>>>>>>>>>>>>>>> 다시 풀어볼 것
